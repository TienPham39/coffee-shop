<template>
  <section class="overflow-hidden cursor-pointer">
    <div class="w-full max-w-none mx-auto overflow-x-hidden">
      <!-- Full-bleed video -->
      ...
      <div class="relative left-1/2 -ml-[50vw] w-screen z-0">
        <div class="bg-[#670000] relative z-0 py-10">
          <!-- Gradient top -->
          <div
          ...
          <div
            ref="scrollContainer"
            class="max-h-[650px] overflow-y-auto scroll-smooth scrollbar-thin scrollbar-thumb-white/30 scrollbar-track-black/20 px-4 md:px-30"
            @mousemove="handleMouseMove"
            @mouseleave="stopAutoScroll"
          >
            <div
              class="grid grid-cols-1 md:grid-cols-3 gap-y-4 justify-items-center"
            >
              <!-- Left column -->
              <div class="flex flex-col gap-3 items-center">
                <img
                  ...
                  :class="[
                    'object-cover rounded-md transition-transform duration-300 hover:scale-105',
                    item.full ? 'w-[380px] h-[540px]' : 'w-[380px] h-[260px]',
                  ]"
                />
              </div>
              ...
              <div class="flex flex-col items-center gap-4">
                <img
                  ...
                  class="w-[220px] md:w-[260px] mb-4 object-contain"
                />
                <div
                  v-for="item in columns[1]"
                  :key="item.video.id"
                  class="relative rounded-md overflow-hidden hover:scale-105 transition-transform duration-300"
                >
                  <img
                    :src="item.video.thumb"
                    :alt="item.video.title"
                    class="object-cover w-[380px] h-[260px]"
                  />
                </div>
              </div>
